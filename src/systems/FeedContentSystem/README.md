...
- **Static model import** — `PostService.toggleLike()` previously used a dynamic `await import()` on every like action, adding async overhead on one of the hottest endpoints. It now uses a top-level static import, which is resolved once at startup.
- **No raw user input in queries** — Feed, hashtag, and search queries rely on `$in` lookups and indexed fields, not user-supplied regex patterns.
